...
def find_nn_cos(v, Wv, k=10):
    ...
    dot_product = np.dot(Wv, v)
    
    other_words_norms = np.linalg.norm(Wv, axis=1)
    this_word_norm = np.linalg.norm(v)
    
    cos_similarity = dot_product/((this_word_norm * other_words_norms).reshape(-1,1))
...
def analogy(vA, vB, vC, Wv, k=5):
    ...
    return find_nn_cos(vB - vA + vC, Wv, k=k)
